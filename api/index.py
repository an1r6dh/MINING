...
def send_alert_email(payload: AlertEmailPayload):
    ...
    subject = f"[{level} MINE HAZARD ALERT - {node}] Evacuate Immediately — {site}"
...
class HardwareTelemetryPayload(BaseModel):
    source: str = "ESP32_S3_HARDWARE"
    node_id: str = "NODE_01"
    filter_mode: str = "KALMAN FILTERED"
    tilt: float = 0.0
    vibration: float = 0.0
...
@app.post("/api/hardware_telemetry")
@app.post("/hardware_telemetry")
def receive_hardware_telemetry(payload: HardwareTelemetryPayload):
    global latest_hardware_telemetry
    now_ts = time.time()
    formatted_time = payload.timestamp or time.strftime("%H:%M:%S")
    ...
    latest_hardware_telemetry["nodes"][payload.node_id] = node_data

    latest_hardware_telemetry["source"] = payload.source
    latest_hardware_telemetry["connected"] = True
    latest_hardware_telemetry["node_id"] = payload.node_id
    latest_hardware_telemetry["filter_mode"] = payload.filter_mode
    latest_hardware_telemetry["tilt"] = payload.tilt
    latest_hardware_telemetry["vibration"] = payload.vibration
    latest_hardware_telemetry["displacement"] = payload.displacement
    latest_hardware_telemetry["status"] = payload.status
    latest_hardware_telemetry["frame_id"] = payload.frame_id
    latest_hardware_telemetry["timestamp"] = formatted_time
    latest_hardware_telemetry["received_at"] = now_ts

    return {"status": "success", "received": latest_hardware_telemetry}

@app.get("/api/hardware_telemetry")
@app.get("/hardware_telemetry")
def get_hardware_telemetry():
    now_ts = time.time()
    is_live = (now_ts - latest_hardware_telemetry.get("received_at", 0)) < 6.0
...
<body>
    ...
    <main>
        ...
        <div id="page-monitoring" class="page-container">
            ...
            <!-- HARDWARE INTEGRATION STATUS BAR (ESP32-S3 HUB & ESP-NOW TDMA NODES) -->
            <div style="background: rgba(15, 23, 42, 0.7); border: 1px solid rgba(56, 189, 248, 0.3); border-radius: 8px; padding: 0.75rem 1.25rem; margin-bottom: 1rem; display: flex; flex-wrap: wrap; justify-content: space-between; align-items: center; gap: 0.75rem; box-shadow: 0 4px 15px rgba(0,0,0,0.25);">
                <div style="display: flex; align-items: center; gap: 12px; flex-wrap: wrap;">
                    <div id="hw-live-badge" style="display: inline-flex; align-items: center; gap: 7px; background: rgba(148, 163, 184, 0.12); border: 1px solid rgba(148, 163, 184, 0.35); padding: 4px 12px; border-radius: 16px; font-size: 0.78rem; font-weight: 700; color: #94a3b8; transition: all 0.3s ease;">
                        <span id="hw-live-dot" style="width: 8px; height: 8px; border-radius: 50%; background: #94a3b8; display: inline-block; box-shadow: 0 0 6px #94a3b8;"></span>
                ...
                <div style="display: flex; align-items: center; gap: 12px; font-size: 0.78rem; color: #94a3b8;">
                    <span>TDMA Superframe: <strong id="hw-frame-id" style="color: #94a3b8; font-family: monospace; font-size: 0.88rem;">#0000 (0.000 Idle)</strong></span>
                    <span style="border-left: 1px solid #334155; height: 16px;"></span>
                    <span id="hw-kalman-status" style="color: #10b981; font-weight: 600;">1D Discrete Kalman Filter Active</span>
                </div>
            </div>
            ...
            <!-- METRICS GRID (HARDWARE TELEMETRY & KALMAN FILTER CHANNELS) -->
            <div class="metrics-grid">
                <div class="metric-card">
                    <div class="metric-header">
                        <span class="metric-title">Filtered Tilt (AngleX)</span>
                    </div>
                    <div class="metric-value"><span id="val-tilt">0.000</span><span style="font-size: 1.1rem; color: var(--text-muted); font-weight: 500;">&deg;</span></div>
                    <div class="metric-footer">
                        <span>MPU6050 I2C (21/22) &bull; kfTilt (Q:0.02, R:1.5)</span>
                        <div id="sub-tilt" style="font-size: 0.72rem; color: #38bdf8; margin-top: 3px;">Raw: 0.000&deg; &bull; Standby (0.000)</div>
                    </div>
                </div>
                <div class="metric-card">
                    <div class="metric-header">
                    ...
                    <div class="metric-value"><span id="val-vib">0.000</span><span style="font-size: 1.1rem; color: var(--text-muted); font-weight: 500;"> g</span></div>
                    <div class="metric-footer">
                        <span>Dynamic Accel &bull; kfVib (Q:0.05, R:2.0)</span>
                        <div id="sub-vib" style="font-size: 0.72rem; color: #f59e0b; margin-top: 3px;">Raw: 0.000g &bull; Standby (0.000)</div>
                    </div>
                </div>
                <div class="metric-card">
                    <div class="metric-header">
                    ...
                    <div class="metric-value"><span id="val-strain">0.000</span><span style="font-size: 1.1rem; color: var(--text-muted); font-weight: 500;"> mm</span></div>
                    <div class="metric-footer">
                        <span>HC-SR04 Ultrasonic (5/18) &bull; kfDisp (Q:0.01)</span>
                        <div id="sub-strain" style="font-size: 0.72rem; color: #10b981; margin-top: 3px;">Raw: 0.000mm &bull; Standby (0.000)</div>
                    </div>
                </div>
                <div class="metric-card">
                    <div class="metric-header">
        ...
        <div id="page-admin" class="page-container" style="display: none;">
            ...
            <!-- LIVE ALERT EMAIL DISPATCH TESTER -->
            <div class="panel-card" style="border: 1px solid rgba(220, 38, 38, 0.4); background: rgba(220, 38, 38, 0.05); margin-bottom: 1.5rem;">
                <div class="panel-header">
                    <div class="panel-title"> <span>📧 Live Alert Email Dispatch Tester</span></div>
                    <span style="font-size: 0.78rem; color: #f87171;">Send real-time test evacuation alerts to any target email address</span>
                </div>
                <div style="padding: 1.2rem;">
                    <p style="font-size: 0.85rem; color: var(--text-secondary); margin-bottom: 1rem;">
                        Test sending the emergency evacuation alert <em>"Alert: you have to move from that current site"</em> from <strong>miningwithigniters@gmail.com</strong> to ANY email address (e.g. personal inbox, temporary/garbage email, or operator email).
                    </p>
                    <div style="display: flex; flex-wrap: wrap; gap: 0.75rem; align-items: flex-end;">
                        ...
                        <div style="flex: 1; min-width: 150px;">
                            <label style="font-size: 0.8rem; color: var(--text-muted); display: block; margin-bottom: 5px; font-weight: 600;">Alert Severity:</label>
                            <select id="test-alert-level" style="width: 100%; padding: 0.6rem 0.85rem; border-radius: 6px; background: var(--bg-card); border: 1px solid var(--border-color); color: var(--text-primary); font-size: 0.88rem; outline: none;">
                                <option value="DANGER">DANGER (Critical Evacuation)</option>
                                <option value="WARNING">WARNING (Threshold Drift)</option>
                            </select>
                        </div>
                        <button type="button" onclick="sendCustomTestAlertEmail()" style="background: #dc2626; color: #fff; font-weight: 700; border: none; padding: 0.65rem 1.4rem; border-radius: 6px; cursor: pointer; display: flex; align-items: center; gap: 6px; height: 39px;">
                            <span>🚀 Send Test Alert Email</span>
                        </button>
                    </div>
                    <div id="test-email-result" style="margin-top: 1rem; display: none; font-size: 0.85rem;"></div>
                </div>
            </div>
    ...
    <script>
        ...
        async function syncUserToSupabase(username, role, status, registeredAt, email) {
            ...
            try {
                ...
                const cleanEmail = (email || (users[username] ? users[username].email : "") || "").trim().toLowerCase();
                const baseReg = (registeredAt || new Date().toLocaleString()).replace(/\[email:[^\]]+\]/, '').replace(/\|\s*email:[^\s\]]+/, '').trim();
                const regPayload = cleanEmail ? `${baseReg} [email:${cleanEmail}]` : baseReg;

                await supabaseClient.from('users').upsert([{
                    username: username,
                    role: role || "Operator",
                    status: status || "Approved",
                    registered_at: regPayload
                }], { onConflict: 'username' });
                console.log(`[SUPABASE] User "${username}" synced with email metadata`);
            } catch(err) {
        ...
        async function deleteUserFromSupabase(username) {
            ...
            try {
                ...
                async function promptEditUserEmail(uname) {
            const users = getUsers();
            const current = (users[uname] && users[uname].email) ? users[uname].email : (DEFAULT_USERS[uname]?.email || "");
            const newEmail = prompt(`Enter or update email address for user "${uname}":`, current);
            ...
            if (!users[uname]) users[uname] = { role: "Operator", status: "Approved" };
            users[uname].email = clean;
            saveUsers(users);
            await syncUserToSupabase(uname, users[uname].role, users[uname].status, users[uname].registeredAt, clean);
            renderAdminPanel();
            alert(`Email address for "${uname}" updated to: ${clean || 'None'}`);
        }

        function broadcastAlertToAllUsers() {
            const list = getAllRegisteredPersonnelEmails();
            if (list.length === 0) {
                alert("No registered user emails found. Please update user emails in the table first.");
                return;
            }
            const confirmed = confirm(`Broadcast emergency evacuation alert from master email (miningwithigniters@gmail.com) to all ${list.length} registered personnel:\n\n${list.join("\n")}\n\nProceed?`);
            if (!confirmed) return;
            dispatchEmergencyAlertEmail("DANGER", 5.24, 2.15, 3.10, list.join(", "));
        }
        ...
        async function renderAdminPanel() {
                    ...
                    <tr>
                        ...
                        <td>
                            <div style="display: inline-flex; align-items: center; gap: 6px;">
                                <span style="color: var(--primary-accent); font-family: monospace; font-size: 0.82rem; font-weight: 600;">${u.email || '<em style="color:#94a3b8;">No email registered</em>'}</span>
                                <button onclick="promptEditUserEmail('${u.username}')" title="Edit Email" style="background: none; border: none; cursor: pointer; color: #94a3b8; font-size: 0.8rem; padding: 2px 4px; border-radius: 4px;">✏️</button>
                            </div>
                        </td>
            ...
            if (Object.keys(users).length === 0) {
                ...
                usersTbody.innerHTML = Object.keys(users).map(uname => {
                    const u = users[uname];
                    const userEmail = u.email || DEFAULT_USERS[uname]?.email || "";
                    ...
                    return `
                        <tr>
                            ...
                            <td>
                                <div style="display: inline-flex; align-items: center; gap: 6px;">
                                    <span style="color: var(--primary-accent); font-family: monospace; font-size: 0.82rem; font-weight: 600;">${userEmail || '<em style="color:#94a3b8;">No email registered</em>'}</span>
                                    <button onclick="promptEditUserEmail('${uname}')" title="Edit Email" style="background: none; border: none; cursor: pointer; color: #94a3b8; font-size: 0.8rem; padding: 2px 4px; border-radius: 4px;" onmouseover="this.style.color='#38bdf8'" onmouseout="this.style.color='#94a3b8'">✏️</button>
                                </div>
                            </td>
                            ...
                            <td>
                                ${uname === 'Admin' ? `
                                    <button onclick="broadcastAlertToAllUsers()" style="background: rgba(220,38,38,0.25); color: #fca5a5; border: 1px solid rgba(220,38,38,0.5); padding: 0.35rem 0.75rem; border-radius: 6px; font-size: 0.78rem; font-weight: 700; cursor: pointer;" title="Send live emergency evacuation alert from miningwithigniters@gmail.com to all registered users">🚨 Broadcast All Users</button>
                                ` : `
                                    <button onclick="triggerManualTestAlert('${userEmail}', '${uname}')" style="background: rgba(220,38,38,0.2); color: #f87171; border: 1px solid rgba(220,38,38,0.4); padding: 0.3rem 0.60rem; border-radius: 6px; font-size: 0.78rem; font-weight: 600; cursor: pointer; margin-right: 0.3rem;" title="Send live evacuation alert from miningwithigniters@gmail.com directly to ${uname} (${userEmail})">🚨 Alert</button>
        ...
        async function approveUser(uname) {
            ...
            if (users[uname]) {
                ...
                saveUsers(users);
                await syncUserToSupabase(uname, users[uname].role, "Approved", users[uname].registeredAt, users[uname].email);
                renderAdminPanel();
            }
        }
        ...
        async function toggleUserStatus(uname) {
            ...
            if (users[uname]) {
                ...
                saveUsers(users);
                await syncUserToSupabase(uname, users[uname].role, newStatus, users[uname].registeredAt, users[uname].email);
                renderAdminPanel();
            }
        }
        ...
        // ====================================================================================================
        // HARDWARE EMBEDDED ENGINE: 1D DISCRETE KALMAN FILTER (MATCHING central_hub_esp32s3_v6.ino)
        // ====================================================================================================
        class DiscreteKalmanFilter1D {
            constructor(processNoise = 0.02, measurementNoise = 1.5, estimationError = 1.0, initialVal = 0.0) {
                this.q = processNoise;
            ...
            update(measurement) {
                ...
                const denominator = this.p + this.r;
                this.k = (Math.abs(denominator) > 1e-6) ? (this.p / denominator) : 0.5;
                this.x = this.x + this.k * (measurement - this.x);
                this.p = (1.0 - this.k) * this.p;
                return this.x;
            }

            reset(initialVal = 0.0) {
                this.x = initialVal;
                this.p = 1.0;
                this.initialized = false;
            }
        ...
        // Dedicated 1D Kalman Filter instances matching Central Hub firmware:
        // kfTilt: Q=0.02, R=1.5
        // kfVibration: Q=0.05, R=2.0
        // kfDisplacement: Q=0.01, R=0.8
        const kfTiltNode1 = new DiscreteKalmanFilter1D(0.02, 1.5);
        ...
        let liveHardwareData = null;

        // Web Serial API Direct Browser Connection (Google Chrome / Microsoft Edge)
        ...
        async function toggleWebSerialConnection() {
            ...
            try {
                ...
                setHardwareBadgeActive("LIVE HARDWARE CONNECTED (Web Serial USB)");
                readWebSerialLoop();
            } catch (err) {
        ...
        async function readWebSerialLoop() {
            ...
            try {
                ...
                while (isWebSerialReading) {
                    ...
                    lineBuf = lines.pop(); // keep partial chunk

                    for (const line of lines) {
                        parseAndApplyHardwareLine(line);
                    }
                }
            } catch (err) {
                console.warn("Web Serial stream error:", err);
            } finally {
                disconnectWebSerial();
            }
        }

        function setHardwareBadgeActive(label) {
            ...
            if (badgeText) badgeText.textContent = label || "LIVE HARDWARE CONNECTED (ESP32-S3 USB)";
            if (badge) {
        ...
        function parseAndApplyHardwareLine(rawLine) {
            ...
            if (tiltMatch && (vibMatch || dispMatch)) {
                ...
                if (selectedNodeId === nodeIdStr || !liveHardwareData.node_id) {
                    ...
                    liveHardwareData.vibration = vibVal;
                    liveHardwareData.displacement = dispVal;
                    liveHardwareData.status = statusVal;
                }
        ...
        function getSimulatedPayload() {
            ...
            if (liveHardwareData && liveHardwareData.connected) {
                ...
                if (activeNodeData) {
                    ...
                    const realDisp = parseFloat((activeNodeData.displacement || 0).toFixed(3));
                    return {
                        node_id: selectedNodeId,
                        raw_tilt: realTilt,
                        raw_vib: realVib,
                        raw_strain: realDisp,
                        filtered_tilt: realTilt,
                        filtered_vibration: realVib,
                        filtered_strain: realDisp,
                        battery: baseBattery,
                        is_hardware: true
                    };
                }
            ...
            // 2. Hardware Disconnected: strictly 0.000 in Dynamic/Safe mode!
            let rawTilt = 0.000;
            let rawVib = 0.000;
            let rawDisp = 0.000;
            ...
            if (selectedNodeId === "NODE_02") {
                ...
                return {
                    ...
                    raw_vib: rawVib,
                    raw_strain: rawDisp,
                    filtered_tilt: rawTilt,
                    filtered_vibration: rawVib,
                    filtered_strain: rawDisp,
                    battery: baseBattery,
                    is_hardware: false
                };
            }
            ...
            return {
                ...
                raw_vib: rawVib,
                raw_strain: rawDisp,
                filtered_tilt: filtTilt,
                filtered_vibration: filtVib,
                filtered_strain: filtDisp,
                battery: baseBattery,
                is_hardware: false
            };
        }
        ...
        // =========================================================================
        // EMERGENCY HAZARD ALERT EMAIL DISPATCHER (MULTI-NODE DANGER TRACKING)
        // =========================================================================
        ...
        function showEmergencyEmailToast(recipients, level, msg, nodeInfo) {
            ...
            if (!toast) {
                ...
                toast.style.cssText = "position: fixed; top: 20px; right: 20px; z-index: 99999; color: #fff; padding: 14px 20px; border-radius: 10px; box-shadow: 0 10px 30px rgba(0,0,0,0.5); font-size: 0.88rem; font-weight: 600; border-left: 6px solid #fef08a; display: flex; align-items: center; gap: 12px; transition: opacity 0.4s ease; backdrop-filter: blur(8px); max-width: 580px;";
            ...
            const nodeLabel = nodeInfo ? `<div style="font-size: 0.82rem; color: #fef08a; margin-bottom: 2px;">⚡ Node Source: <strong>${nodeInfo}</strong></div>` : "";
        ...
        async function dispatchEmergencyAlertEmail(level, tilt, vib, strain, forcedRecipient, customNodeId, customMessage) {
            try {
                let recipientList = [];
                if (forcedRecipient) {
                ...
                showEmergencyEmailToast(recipientList, level, targetMsg, targetNode);
            } catch (err) {
                ...
                showEmergencyEmailToast(fallbackList, level, customMessage || "Alert: you have to move from that current site", customNodeId || selectedNodeId);
            }
        }

        function triggerManualTestAlert(targetEmail, targetName) {
            let dest = (targetEmail || "").trim();
            if (!dest || !dest.includes("@")) {
                const entered = prompt(`Enter recipient email address to send live evacuation alert to ${targetName || 'user'}:`);
                if (!entered) return;
                dest = entered.trim();
                if (targetName) {
                    const users = getUsers();
                    if (users[targetName]) {
                        users[targetName].email = dest;
                        saveUsers(users);
                        syncUserToSupabase(targetName, users[targetName].role, users[targetName].status, users[targetName].registeredAt, dest);
                        renderAdminPanel();
                    }
                }
            }
            dispatchEmergencyAlertEmail("DANGER", 5.24, 2.15, 3.10, dest);
        }

        async function sendCustomTestAlertEmail() {
            const emailInput = document.getElementById("test-recipient-email");
            const levelSelect = document.getElementById("test-alert-level");
            const resDiv = document.getElementById("test-email-result");

            const email = (emailInput ? emailInput.value : "").trim();
            const level = levelSelect ? levelSelect.value : "DANGER";
            ...
            if (resDiv) {
                ...
                resDiv.innerHTML = `<span style="color: #38bdf8;">⏳ Transmitting emergency evacuation alert to <strong>${email}</strong> via miningwithigniters@gmail.com...</span>`;
            }
            ...
            try {
                const payload = {
                    ...
                    vibration: level === "DANGER" ? 2.15 : 0.45,
                    strain: level === "DANGER" ? 3.10 : 0.70
                };
                ...
                if (res.ok && data.status === "success") {
                    const deliveryNote = data.delivered 
                        ? `<span style="color: #10b981; font-weight: bold;">✅ REAL EMAIL DELIVERED! Sent via Gmail SMTP straight to <strong>${email}</strong>'s inbox!</span>`
                        : `<span style="color: #10b981; font-weight: bold;">✅ Alert Dispatched: Sent from <strong>miningwithigniters@gmail.com</strong> to <strong>${email}</strong>.</span>`;
                    if (resDiv) {
                        resDiv.innerHTML = `
                            <div style="background: rgba(0,0,0,0.3); padding: 10px 14px; border-radius: 6px; border: 1px solid rgba(255,255,255,0.1);">
                                ${deliveryNote}
                                <div style="margin-top: 5px; font-size: 0.8rem; color: #94a3b8;">
                                    From: <strong>miningwithigniters@gmail.com</strong> &bull; Message: <em>"Alert: you have to move from that current site"</em> &bull; Time: ${data.timestamp}
                                </div>
                            </div>
                        `;
                    }
                    showEmergencyEmailToast(email, level, "Alert: you have to move from that current site");
                } else {
                    if (resDiv) resDiv.innerHTML = `<span style="color: #ef4444;">❌ Dispatch notice: ${data.detail || 'Service response check'}</span>`;
                }
            } catch(err) {
                if (resDiv) resDiv.innerHTML = `<span style="color: #ef4444;">❌ Notice: ${err.message}</span>`;
            }
        }
        ...
        function updateTelemetry() {
            ...
            // =========================================================================
            // MULTI-NODE HAZARD EVALUATION (NODE_01, NODE_02, OR BOTH IN DANGER)
            // =========================================================================
            const currentSiteObj = MINING_SITES[selectedSiteKey] || MINING_SITES["site-1"];
            const node01Obj = (currentSiteObj && currentSiteObj.nodes) ? currentSiteObj.nodes.find(n => n.id === "NODE_01") : null;
            const node02Obj = (currentSiteObj && currentSiteObj.nodes) ? currentSiteObj.nodes.find(n => n.id === "NODE_02") : null;

            if (selectedNodeId === "NODE_01" && node01Obj) {
                node01Obj.status = status;
                node01Obj.tilt = tiltVal;
                node01Obj.vib = vibVal;
                node01Obj.strain = strainVal;
            } else if (selectedNodeId === "NODE_02" && node02Obj) {
                node02Obj.status = status;
                node02Obj.tilt = tiltVal;
                node02Obj.vib = vibVal;
                node02Obj.strain = strainVal;
            }
            ...
            const isNode1Danger = Boolean(node01Obj && node01Obj.status === "DANGER");
            const isNode2Danger = Boolean(node02Obj && node02Obj.status === "DANGER");
            ...
            if (isBothDanger) {
                ...
                if (lastDangerNodeState !== "BOTH" || (nowTs - lastDangerAlertTimestamp > ALERT_EMAIL_COOLDOWN_MS)) {
                    lastDangerNodeState = "BOTH";
                    lastDangerAlertTimestamp = nowTs;
                    lastAlertEmailLevel = "DANGER";
                    const maxT = Math.max(node01Obj ? node01Obj.tilt : 0, node02Obj ? node02Obj.tilt : 0);
                    const maxV = Math.max(node01Obj ? node01Obj.vib : 0, node02Obj ? node02Obj.vib : 0);
                    const maxS = Math.max(node01Obj ? node01Obj.strain : 0, node02Obj ? node02Obj.strain : 0);
                    const bothMsg = "CRITICAL MULTI-ZONE DANGER ALERT: Evacuate site immediately! Both Sensor Node 1 (Deep Rock Extensometer) and Sensor Node 2 (SW-420 Tripwire Digital Latch) have breached critical hazard thresholds simultaneously!";
                    dispatchEmergencyAlertEmail("DANGER", maxT, maxV, maxS, null, "BOTH NODE_01 & NODE_02 (Simultaneous Breach)", bothMsg);
                }
            } else if (isNode1Danger) {
                // CASE 1: NODE_01 IN DANGER
                if (lastDangerNodeState !== "NODE_01" || (nowTs - lastDangerAlertTimestamp > ALERT_EMAIL_COOLDOWN_MS)) {
                    lastDangerNodeState = "NODE_01";
                    lastDangerAlertTimestamp = nowTs;
                    lastAlertEmailLevel = "DANGER";
                    const n1Msg = "DANGER ALERT: Alert: you have to move from that current site! Immediate ground collapse risk breached on NODE_01 (Deep Rock Mass Extensometer).";
                    dispatchEmergencyAlertEmail("DANGER", node01Obj.tilt, node01Obj.vib, node01Obj.strain, null, "NODE_01 (Deep Rock Mass Extensometer)", n1Msg);
                }
            } else if (isNode2Danger) {
                // CASE 2: NODE_02 IN DANGER
                if (lastDangerNodeState !== "NODE_02" || (nowTs - lastDangerAlertTimestamp > ALERT_EMAIL_COOLDOWN_MS)) {
            ...
            // Hardware Metrics Display (Kalman Filtered & Raw Channels)
