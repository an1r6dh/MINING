...
DEFAULT_API_URL = os.getenv("API_URL") or os.getenv("BACKEND_URL") or "http://127.0.0.1:8000/predict"
...
st.sidebar.markdown("🧠 **AI Model**: `25M-RTX4050-GPU-XGBoost` (99.65% Acc)")
...
nav_options = ["🖥️ Live Monitoring", "📊 Telemetry Logs & Analytics", "🗺️ Sites"]
...
# Sidebar Backend Endpoint Config
st.sidebar.header("🌐 Backend Endpoint")
api_url_input = st.sidebar.text_input(
    "API / Tunnel Target URL",
    value=st.session_state.get("custom_api_url", DEFAULT_API_URL),
    help="FastAPI / predict URL (e.g. http://127.0.0.1:8000/predict or Cloudflare Tunnel https://xxxx.trycloudflare.com/predict)"
)
st.session_state["custom_api_url"] = api_url_input
...
def fetch_prediction(payload):
    ...
    clf = load_local_model()
    if clf is not None:
        try:
            df_in = pd.DataFrame(
                [[payload["filtered_tilt"], payload["filtered_vibration"], payload["filtered_strain"]]],
            ...
            try:
                val = int(pred)
                return class_map.get(val, str(pred))
    ...
    vib = payload["filtered_vibration"]
...
elif page_view == "🗺️ Sites":
    st.title("🗺️ Mining Sites & IoT Node Telemetry Map")
    st.markdown("Select Mine Location, Track Active/Inactive Sensor Overlays & Real-Time Hazard Status.")
...
elif page_view == "📊 Telemetry Logs & Analytics":
