...
<body>
        ...
        <div id="page-monitoring" class="page-container">
            ...
            <div style="background: rgba(15, 23, 42, 0.7); border: 1px solid rgba(56, 189, 248, 0.3); border-radius: 8px; padding: 0.75rem 1.25rem; margin-bottom: 1rem; display: flex; flex-wrap: wrap; justify-content: space-between; align-items: center; gap: 0.75rem; box-shadow: 0 4px 15px rgba(0,0,0,0.25);">
                <div style="display: flex; align-items: center; gap: 12px; flex-wrap: wrap;">
                    <div id="hw-live-badge" style="display: inline-flex; align-items: center; gap: 7px; background: rgba(56, 189, 248, 0.12); border: 1px solid rgba(56, 189, 248, 0.4); padding: 4px 12px; border-radius: 16px; font-size: 0.78rem; font-weight: 700; color: #38bdf8; transition: all 0.3s ease;">
                        <span id="hw-live-dot" style="width: 8px; height: 8px; border-radius: 50%; background: #38bdf8; display: inline-block; box-shadow: 0 0 8px #38bdf8;"></span>
                        <span id="hw-live-text">PRE-HARDWARE VIRTUAL TDMA ENGINE</span>
                    </div>
                ...
                <div style="display: flex; align-items: center; gap: 12px; font-size: 0.78rem; color: #94a3b8;">
                    <span>TDMA Superframe: <strong id="hw-frame-id" style="color: #38bdf8; font-family: monospace; font-size: 0.88rem;">#0001</strong> (1000ms)</span>
            ...
            <div class="metrics-grid">
                <div class="metric-card">
                    ...
                    <div class="metric-value"><span id="val-tilt">0.00</span><span style="font-size: 1.1rem; color: var(--text-muted); font-weight: 500;">&deg;</span></div>
                    <div class="metric-footer">
                        <span>MPU6050 I2C (21/22) &bull; kfTilt (Q:0.02, R:1.5)</span>
                        <div id="sub-tilt" style="font-size: 0.72rem; color: #38bdf8; margin-top: 3px;">Raw: 0.00&deg; &bull; Filtered</div>
                    </div>
                </div>
                <div class="metric-card">
                    <div class="metric-header">
                    ...
                    <div class="metric-value"><span id="val-vib">0.00</span><span style="font-size: 1.1rem; color: var(--text-muted); font-weight: 500;"> g</span></div>
                    <div class="metric-footer">
                        <span>Dynamic Accel &bull; kfVib (Q:0.05, R:2.0)</span>
                        <div id="sub-vib" style="font-size: 0.72rem; color: #f59e0b; margin-top: 3px;">Raw: 0.00g &bull; Dynamic</div>
                    </div>
                </div>
                <div class="metric-card">
                    <div class="metric-header">
                    ...
                    <div class="metric-value"><span id="val-strain">0.0</span><span style="font-size: 1.1rem; color: var(--text-muted); font-weight: 500;"> mm</span></div>
                    <div class="metric-footer">
                        <span>HC-SR04 Ultrasonic (5/18) &bull; kfDisp (Q:0.01)</span>
                        <div id="sub-strain" style="font-size: 0.72rem; color: #10b981; margin-top: 3px;">Raw: 0.0mm &bull; Subsidence</div>
                    </div>
                </div>
                <div class="metric-card">
                    <div class="metric-header">
                ...
                <div style="padding: 1.2rem;">
                    ...
                    <div style="display: flex; flex-wrap: wrap; gap: 0.75rem; align-items: flex-end;">
                        <div style="flex: 2; min-width: 250px;">
                            <label style="font-size: 0.8rem; color: var(--text-muted); display: block; margin-bottom: 5px; font-weight: 600;">Destination Email Address:</label>
                            <input type="email" id="test-recipient-email" placeholder="e.g. test@gmail.com, your_email@domain.com" style="width: 100%; padding: 0.6rem 0.85rem; border-radius: 6px; background: var(--bg-card); border: 1px solid var(--border-color); color: var(--text-primary); font-size: 0.88rem; outline: none;">
                        </div>
    ...
    <script>
        ...
        async function checkLiveHardwareStream() {
            ...
            // Pre-Hardware Virtual Mode
            if (Date.now() - lastLiveHardwareSync > 4000) {
                liveHardwareData = null;
                const badge = document.getElementById("hw-live-badge");
                const badgeText = document.getElementById("hw-live-text");
                const badgeDot = document.getElementById("hw-live-dot");
                if (badgeText) badgeText.textContent = "PRE-HARDWARE VIRTUAL TDMA ENGINE";
                if (badge) {
        ...
        function getSimulatedPayload() {
            tdmaVirtualFrameId++;
            const hwFrame = document.getElementById("hw-frame-id");
            if (hwFrame && !liveHardwareData) hwFrame.textContent = "#" + String(tdmaVirtualFrameId % 9999).padStart(4, "0");
            ...
            if (liveHardwareData && liveHardwareData.connected) {
                return {
                    ...
                    raw_tilt: liveHardwareData.tilt,
                    raw_vib: liveHardwareData.vibration,
                    raw_strain: liveHardwareData.displacement,
                    filtered_tilt: liveHardwareData.tilt,
                    filtered_vibration: liveHardwareData.vibration,
                    filtered_strain: liveHardwareData.displacement,
                    battery: baseBattery,
                    is_hardware: true
                };
            }

            // 2. Pre-Hardware Virtual TDMA Mode (faithful implementation of sensor_node_esp32_5.ino math)
            let rawTilt = 0.0;
            let rawVib = 0.0;
            let rawDisp = 0.0;

            if (selectedNodeId === "NODE_02") {
                // Node 2 is a Discrete Tripwire Node (SW-420 sensor) with Digital Override (bypasses Kalman)
                if (activeSimMode === "danger") {
                    rawVib = 1.00; // Tripwire latched
                    rawTilt = 5.20;
                    rawDisp = 16.5;
                    ...
                    // Safe tripwire quiescent baseline
                    rawVib = 0.00;
                    rawTilt = 0.00;
                    rawDisp = 0.00;
                }
            ...
            // NODE_01: Geotechnical Node (MPU6050 + HC-SR04 through 1D Kalman Filters)
            if (activeSimMode === "manual") {
                rawTilt = parseFloat(document.getElementById("slider-tilt").value || 0);
                rawVib = parseFloat(document.getElementById("slider-vib").value || 0);
                rawDisp = parseFloat(document.getElementById("slider-strain").value || 0);
            } else if (activeSimMode === "danger") {
                // Critical strata collapse scenario (all 3 sensors breach)
                rawTilt = 5.35 + (Math.random() * 0.4 - 0.2);
                rawVib = 0.95 + (Math.random() * 0.2 - 0.1);
                rawDisp = 16.8 + (Math.random() * 0.6 - 0.3);
            } else if (activeSimMode === "warning") {
                // Moderate geotechnical drift
                rawTilt = 2.90 + (Math.random() * 0.2 - 0.1);
                rawVib = 0.48 + (Math.random() * 0.1 - 0.05);
                rawDisp = 9.4 + (Math.random() * 0.4 - 0.2);
            } else {
                // Normal baseline: MPU6050 stationary with slight physical noise
                rawTilt = 0.05 + (Math.random() * 0.08 - 0.04);
                rawVib = 0.02 + (Math.random() * 0.03);
                rawDisp = 0.3 + (Math.random() * 0.2 - 0.1);
                if (rawDisp < 0) rawDisp = 0.0;
            }

            // Apply Discrete Kalman Filters matching central_hub_esp32s3_v6.ino
            const filtTilt = kfTiltNode1.update(rawTilt);
            const filtVib = kfVibNode1.update(rawVib);
            const filtDisp = kfDispNode1.update(rawDisp);
            ...
            return {
                ...
                raw_vib: rawVib,
                raw_strain: rawDisp,
                filtered_tilt: parseFloat(filtTilt.toFixed(2)),
                filtered_vibration: parseFloat(filtVib.toFixed(2)),
                filtered_strain: parseFloat(filtDisp.toFixed(1)),
                battery: baseBattery,
                is_hardware: false
            };
        }
        ...
        function updateTelemetry() {
            ...
            if (elTilt) elTilt.textContent = tiltVal.toFixed(2);
            const elVib = document.getElementById("val-vib");
            if (elVib) elVib.textContent = vibVal.toFixed(2);
            const elStrain = document.getElementById("val-strain");
            if (elStrain) elStrain.textContent = strainVal.toFixed(1);
            ...
            if (subT) subT.textContent = `Raw: ${rawT.toFixed(2)}° • Filtered`;
            const subV = document.getElementById("sub-vib");
            if (subV) subV.textContent = `Raw: ${rawV.toFixed(2)}g • Dynamic`;
            const subS = document.getElementById("sub-strain");
            if (subS) subS.textContent = `Raw: ${rawS.toFixed(1)}mm • Subsidence`;
