...
def get_hardware_telemetry():
    is_live = (time.time() - latest_hardware_telemetry.get("received_at", 0)) < 5.0
...
<body>
            ...
            <div style="background: rgba(15, 23, 42, 0.7); border: 1px solid rgba(56, 189, 248, 0.3); border-radius: 8px; padding: 0.75rem 1.25rem; margin-bottom: 1rem; display: flex; flex-wrap: wrap; justify-content: space-between; align-items: center; gap: 0.75rem; box-shadow: 0 4px 15px rgba(0,0,0,0.25);">
                <div style="display: flex; align-items: center; gap: 12px; flex-wrap: wrap;">
                    ...
                    <span style="font-size: 0.82rem; color: #94a3b8;">
                        Hub: <strong style="color: #f1f5f9;">ESP32-S3 (central_hub_esp32s3_v6)</strong> &bull;
    ...
    <script>
        ...
        // Hardware safety thresholds strictly matching central_hub_esp32s3_v6.ino
        ...
        async function checkLiveHardwareStream() {
            try {
                ...
                if (res.ok) {
                    ...
                    if (data && data.connected) {
                        ...
                        if (badgeText) badgeText.textContent = "LIVE HARDWARE CONNECTED (ESP32-S3 USB)";
                        if (badge) {
            ...
            if (Date.now() - lastLiveHardwareSync > 4000) {
        ...
        function getSimulatedPayload() {
            ...
            // 1. If physical ESP32-S3 hardware is broadcasting via serial bridge, use its real readings!
            if (liveHardwareData && liveHardwareData.connected) {
                if (hwFrame) hwFrame.textContent = "#" + String(liveHardwareData.frame_id || (tdmaVirtualFrameId % 9999)).padStart(4, "0");
                const realTilt = parseFloat((liveHardwareData.tilt || 0).toFixed(3));
                const realVib = parseFloat((liveHardwareData.vibration || 0).toFixed(3));
                const realDisp = parseFloat((liveHardwareData.displacement || 0).toFixed(3));
                return {
                    node_id: liveHardwareData.node_id || selectedNodeId,
                    raw_tilt: realTilt,
                    raw_vib: realVib,
                    raw_strain: realDisp,
